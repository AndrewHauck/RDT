...
#layout = valves + COMSelector + diagram + stages + readings
layout = valves + COMSelector + diagram + stages + readings

# Valve states
isValveOpen = [False, False, False, False, False, False]
# Create the Window and Finalize it. Then fullscreen
window = sg.Window('RocketView', layout, grab_anywhere=False)
...
while True:
        ...
        if not ser.is_open:
            try:    #OPENING SERIAL PORT
                ...
                if __name__ == '__main__':
                    ...
                    window.FindElement('Enquire').update(button_color=('White', 'Red'))
            except serial.SerialException: #IF PORT CAN'T BE OPENED
                print("Unable to Open Port")
        else:   #CLOSING SERIAL PORT
            ser.close()
            window.FindElement('COM_Combo').update(values=ListComPorts.serial_ports())
    ...
    if ser.is_open:
        while ser.in_waiting:
            ...
            if(inChar == chr(6)):
                buffer = ""
                window.FindElement('Enquire').update(button_color=('White', 'Green'))
            if(inChar == chr(4)):
